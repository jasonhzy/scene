...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.18精华设计id && obj.18精华设计id.length > 0) {
                    t.initialize(options, obj.18精华设计id, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"18精华设计id":"' + obj.18精华设计id + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, 18精华设计id) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&18精华设计id=' + 18精华设计id + "&AId=" + AId, true);
    },
    StartPolling: function (AId, 18精华设计id) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, 18精华设计id);
    },
    GetPolling: function (AId, 18精华设计id) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&18精华设计id=' + 18精华设计id + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.18精华设计id) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.18精华设计id, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.18精华设计id);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, 18精华设计id, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, 18精华设计id); });
        }
    ...
    socketconnect: function (options, 18精华设计id) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, 18精华设计id);
                        }
                    }
                });
            });
