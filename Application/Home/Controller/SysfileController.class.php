...
class SysfileController extends Controller {
	...
	public function upload(){
			...
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://yqx.cn/'.$v['path']);
				$datathumb = file_get_contents('http://yqx.cn/'.$v['tmbPath']);
				$filetime = time();
	...
	public function createsyspage(){
			...
			$jsonresult = json_decode($jsonstr,true);
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://yqx.cn/'.$v['properties']['thumbSrc']);
				$filetime = time();
	...
	public function createsysscene(){
			...
			$jsonresult = json_decode($jsonstr,true);
			foreach($jsonresult['list'] as $v){
				$data = file_get_contents('http://yqx.cn/'.$v['image']['imgSrc']);
				$filetime = time();
				...
				{
					$data = file_get_contents('http://yqx.cn/'.$v['image']['bgAudio']['url']);
					$filetime = time();
...
//	}
    ...
    public function createtplurl() {
		...
		foreach($m_scenelist as $v){
			echo "http://yqx.cn/eqs/s/".$v['eqcode']."\n";
		}
	}
